    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <AnimatePresence>
        {showTooltip && !dismissed && (
          <motion.div
            ...
            className="relative bg-[#0F0F16] border border-red-500/30 rounded-2xl p-3 pr-8 shadow-2xl shadow-black/50 max-w-[200px]"
          >
            {/* Dismiss */}
            <button
            ...
            {/* Arrow */}
            <div className="absolute bottom-[-7px] right-5 w-3 h-3 bg-[#0F0F16] border-r border-b border-red-500/30 rotate-45" />
      ...
      <Link to="/chat">
        <motion.button
          ...
          className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-red-600 to-red-500 text-white shadow-2xl shadow-red-900/50 flex items-center justify-center group overflow-hidden"
          aria-label="Open AI Chatbot"
        >
          {/* Glow ring */}
          <span className="absolute inset-0 rounded-2xl ring-2 ring-red-500/40 group-hover:ring-red-400/60 transition-all duration-300" />
          {/* Pulse */}
          <span className="absolute inset-0 rounded-2xl bg-red-500/30 animate-ping" style={{ animationDuration: '2.5s' }} />
